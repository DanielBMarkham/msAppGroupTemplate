...
*)
    ...
    /// Get rid of any junk
    type GetappProgramConfigType=string [] -> seq<string>->appConfigType
    /// Process any args you can from the command line
    /// Get rid of any junk
    ...
    let appProgramHelp = [|"EasyAM. An analysis compiler. It compiles freeform notes to useful project stuff."|]
    //createNewBaseOptions programName programTagLine programHelpText verbose
    let defaultappBaseOptions = createNewBaseOptions "app" "The world's first" appProgramHelp defaultVerbosity
    let defaultappConfig:appConfigType ={ConfigBase = {defaultappBaseOptions with Verbosity=defaultVerbosity}; IncomingStream=[||]; FileListFromCommandLine=[||]}
    let loadappConfigFromCommandLine:GetappProgramConfigType = (fun args incomingStream->
      ...
      let newConfigBase = {defaultappBaseOptions with Verbosity=defaultVerbosity}
      let newVerbosity =ConfigEntryType<_>.populateValueFromCommandLine(defaultVerbosity, args)
    ...
    /// Process any args you can from the command line
    /// Get rid of any junk
    type GetappRProgramConfigType=string [] -> seq<string>->appRConfigType
    // A couple of prototype EAR types to begin thinking about what goes here
    let appRProgramHelp = [|"app Reporting (appR). Takes app master files and translates output."|]
    //createNewBaseOptions programName programTagLine programHelpText verbose
    let defaultappRBaseOptions = createNewBaseOptions "ear" "The reporting engine for EasyAM" appRProgramHelp defaultVerbosity
    let defaultappRConfig:appRConfigType = {ConfigBase = {defaultappRBaseOptions with Verbosity=defaultVerbosity}; IncomingStream=[||]; FileListFromCommandLine=[||]}
    let loadappRConfigFromCommandLine:GetappRProgramConfigType = (fun args incomingStream->
        logEvent Verbose "Method loadappRConfigFromCommandLine beginning....." moduleLogger
        if args.Length>0 && (args.[0]="?"||args.[0]="/?"||args.[0]="-?"||args.[0]="--?"||args.[0]="help"||args.[0]="/help"||args.[0]="-help"||args.[0]="--help") then raise (UserNeedsHelp args.[0]) else
        let newVerbosity =ConfigEntryType<_>.populateValueFromCommandLine(defaultVerbosity, args)
        let newConfigBase = {defaultappRBaseOptions with Verbosity=defaultVerbosity}
        let newVerbosity =ConfigEntryType<_>.populateValueFromCommandLine(defaultVerbosity, args)
        if newVerbosity.parameterValue<>defaultVerbosity.parameterValue
          then
            logEvent Info ("New Verbosity set in loadappRConfigFromCommandLine: " + newVerbosity.parameterValue.ToString()) moduleLogger
            logary.switchLoggerLevel ("", newVerbosity.parameterValue.ToLogLevel())
          else ()
        // Go through the arg list. If it's a file, add to list.
        // If it's a directory, add files in the directory to the list
        ...
        logEvent Verbose "..... Method loadappRConfigFromCommandLine ending. Normal Path." moduleLogger
        turnOnLogging()
        {ConfigBase = newConfigBase; IncomingStream=incomingStream; FileListFromCommandLine=newFilesReferncedFromTheCommandLine}
        )
